    ...
    stages {
            ...
            steps {
                script {
                    bat """
                        ...
                        wsl git remote set-url origin https://%DOCKERHUB_USERNAME%:%GITHUB_TOKEN%@github.com/ben14132-01/java-app-4.git
            ...
            steps {
                script {
                    def response = bat(
                        ...
                        curl -X POST ^
                        -u admin:admin ^
                        "http://localhost:9001/api/projects/create?project=java-app-4&name=java-app-4"
                        ''',
                        returnStatus: true
                    )
        ...
        stage('SonarQube Analysis') {
            ...
            steps {
                dir('src/SpringAPI') {
                    bat '''
                    mvn clean verify sonar:sonar "-Dsonar.projectKey=java-app-4" "-Dsonar.projectName='java-app-4'" "-Dsonar.host.url=http://localhost:9001" "-Dsonar.token=%SONAR_TOKEN%"
                    '''
                }
            }
        }
        ...
        stage('Build and Push Docker Image') {
            steps {
                script {
                    def appName = 'java-app-4' //need to change appname
                    bat """
        ...
        //             bat """
        //                 wsl helm upgrade --install java-app ./charts/java-app-4 -n java --create-namespace --set image.tag=${commitId}
        //             """
        //         }
        //     }
        // }
 ...
 //add argocd password to jenkins credentials
        ...
        stage('ArgoCD App Sync') {
            steps {
                script {
                    bat """
                        wsl argocd app sync java-app-4
                        wsl argocd app wait java-app-4 --timeout 180
                    """
                }
            }
        }
